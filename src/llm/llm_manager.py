        ...
        while True:
            try:
                ...
                if e.response.status_code == 429:
                    ...
                    logger.error(f"HTTP error occurred with status code: {e.response.status_code}, waiting 10 seconds before retrying")
                    time.sleep(10)

            except Exception as e:
                logger.error(f"Unexpected error occurred: {str(e)}")
                logger.info(
                    "Waiting for 10 seconds before retrying due to an unexpected error.")
                time.sleep(10)
                continue
