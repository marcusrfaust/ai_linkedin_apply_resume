...
class AIHawkEasyApplier:
    ...
    def job_apply(self, job: Any):
        ...
        try:
            ...
            # Se a sessão estiver inválida, tentar reabrir a página base de jobs e depois a vaga
            if "invalid session id" in str(e).lower():
                try:
                    logger.warning("WebDriver session invalid while navigating to job link. Attempting light recovery.")
                    self.driver.get("https://www.linkedin.com/jobs/")
                    time.sleep(2)
                    self.driver.get(job.link)
                    logger.debug(f"Re-tried navigation to job link: {job.link}")
                except Exception as nav_err:
                    logger.error(f"Recovery navigation failed: {nav_err}")
                    raise
            else:
                raise
        ...
        try:
            ...
            try:
                ActionChains(self.driver).move_to_element(easy_apply_button).pause(random.uniform(0.2, 0.6)).click(easy_apply_button).perform()
            except Exception:
                # Fallback direto com JS
                try:
                    self.driver.execute_script("arguments[0].click();", easy_apply_button)
                except Exception as click_err:
                    logger.error(f"Failed to click 'Easy Apply' button via ActionChains and JS: {click_err}")
                    raise
            ...
            # fechar modal se aberto (substitui _discard_application())
            try:
                close_button = WebDriverWait(self.driver, 3).until(
                    EC.element_to_be_clickable((By.XPATH, '//button[contains(@aria-label, "Dismiss")] | //button[contains(@aria-label, "Close")] | //button[@data-test-modal-close]'))
                )
    ...
    def _find_easy_apply_button(self, job: Any) -> WebElement:
        ...
        while attempt < 2:
            ...
            self.check_for_premium_redirect(job)
            # inline scroll (substitui _scroll_page())
            try:
                scrollable = self.driver.find_element(By.TAG_NAME, 'html')
                for _ in range(random.randint(2, 4)):
                    self.driver.execute_script("arguments[0].scrollBy(0, arguments[1]);", scrollable, random.randint(200, 600))
    ...
    def _get_job_description(self) -> str:
        ...
        try:
            # Tentar múltiplos seletores para descrição da vaga
            ...
            for selector in description_selectors:
                try:
                    ...
                    logger.debug(f"Job description found using selector: {selector}")
                    break
                except NoSuchElementException:
                    continue
            
            if not description:
                # Tentar por XPath como fallback
                xpath_selectors = [
                    '//div[contains(@class, "jobs-description-content")]',
                    '//div[contains(@class, "job-details-about-the-job-module")]',
                ...
                for xpath in xpath_selectors:
                    try:
                        ...
                        logger.debug(f"Job description found using XPath: {xpath}")
                        break
                    except NoSuchElementException:
                        continue
            
            if not description:
                # Se não conseguir encontrar a descrição, usar uma descrição padrão
                logger.warning("Job description not found, using default description")
                description = "Job description not available. Please refer to the job posting for details."

            logger.debug("Job description retrieved successfully")
            return description
        except Exception as e:
            logger.warning(f"Could not extract job description: {e}")
            return "Job description not available. Please refer to the job posting for details."

    def _get_job_recruiter(self):
        logger.debug("Getting job recruiter information")
        try:
            # Tenta padrões comuns do LinkedIn para área "Posted by"/recrutador
            selectors = [
                (By.CSS_SELECTOR, 'a.jobs-poster__name'),
                (By.CSS_SELECTOR, 'div.jobs-poster__name a'),
                (By.XPATH, '//a[contains(@href, "/in/") and (contains(@class, "poster") or contains(@class, "recruit") or contains(@class, "ember-view"))]'),
                (By.XPATH, '//section[contains(@class, "jobs-poster") or contains(@class, "jobs-poster__container")]//a[contains(@href, "/in/")]'),
                (By.XPATH, '//span[contains(text(), "Posted by") or contains(text(), "Recruiter")]//ancestor::section//a[contains(@href, "/in/")]'),
            ]

            for by, expr in selectors:
                try:
                    el = WebDriverWait(self.driver, 2).until(EC.presence_of_element_located((by, expr)))
                    href = el.get_attribute('href') or ''
                    text = el.text.strip() if hasattr(el, 'text') else ''
                    recruiter = href or text
                    if recruiter:
                        logger.debug(f"Recruiter link/name found: {recruiter}")
                        return recruiter
                except Exception:
                    continue

            # Fallback: tenta capturar bloco de "Posted by" e extrair primeiro link de perfil
            try:
                block = self.driver.find_element(By.XPATH, '//section[contains(@class, "jobs-poster") or contains(., "Posted by")]')
                link = block.find_element(By.XPATH, './/a[contains(@href, "/in/")]')
                recruiter = link.get_attribute('href') or ''
                if recruiter:
                    logger.debug(f"Recruiter link found (fallback): {recruiter}")
                    return recruiter
            except Exception:
                pass

            logger.debug("Recruiter not found; returning empty string")
            return ""
        except Exception as e:
            logger.debug(f"Error while retrieving recruiter info: {e}")
            return ""
